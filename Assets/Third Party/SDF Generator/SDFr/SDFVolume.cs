...
namespace SDFr
{
    public class SDFVolume : AVolume<SDFVolume>
    {
        ...
        private RayTracingShader m_SDFGenerateShader;
        private int m_VoxelNum;
        private vec3 m_HalfVoxel;
        private ComputeBuffer m_OutputsBuffer;
        public void BakeOnGPU(int raySamples, Action<SDFVolume, float[], float, object> bakeComplete)
        {
            onBakeComplete = bakeComplete;
            Stopwatch Timer = new Stopwatch();
            Timer.Start();

            m_VoxelNum = _settings.CellCount;
            float[] distances = new float[m_VoxelNum];
...
#if UNITY_EDITOR
            m_SDFGenerateShader = Resources.Load<RayTracingShader>("Shaders/SDFGenerator");
#endif
            float[] m_OutDistances = new float[m_VoxelNum];
            for (uint i = 0; i < m_VoxelNum; i++)
            {
                m_OutDistances[i] = 0.0f;
            }
            m_OutputsBuffer = new ComputeBuffer(m_VoxelNum, sizeof(float));
            m_OutputsBuffer.SetData(m_OutDistances);
            m_HalfVoxel = _settings.HalfVoxel;

            m_AccelerationStructure.Build();
            m_SDFGenerateShader.SetAccelerationStructure("_AccelerationStructure", m_AccelerationStructure);
            m_SDFGenerateShader.SetShaderPass("SDFHit");

            m_SDFGenerateShader.SetBuffer("_OutputBuffer", m_OutputsBuffer);
            m_SDFGenerateShader.SetVector("_HalfVoxel", m_HalfVoxel);
            m_SDFGenerateShader.SetVector("_VolumeSize", _settings.BoundsLocal.size);
            m_SDFGenerateShader.SetVector("_VoxelSize", _settings.VoxelSize);
            m_SDFGenerateShader.SetFloats("_Dimensions", _settings.Dimensions.x, _settings.Dimensions.y, _settings.Dimensions.z);
            m_SDFGenerateShader.SetMatrix("_LocalToWorld", LocalToWorldNoScale);           
            m_SDFGenerateShader.SetInt("_RaySamples", raySamples);

            m_SDFGenerateShader.Dispatch("SDFRayGen", _settings.Dimensions.x, _settings.Dimensions.y, _settings.Dimensions.z);//后面的三个参数是需要发光线的格子

            m_OutputsBuffer.GetData(distances);

            Timer.Stop();
            Debug.Log("SDF baked on GPU completed in " + Timer.Elapsed.ToString("mm\\:ss\\.ff"));

            float aabbMagnitude = BoundsWorldAABB.size.magnitude;
            onBakeComplete?.Invoke(this, distances, aabbMagnitude, null);
        }

        public void BakeOnCPU( int raySamples, List<Renderer> renderers, Action<SDFVolume,float[],float,object> bakeComplete, object passthrough = null )
        {
            onBakeComplete = bakeComplete;
            ...
            stopwatch.Stop();
            Debug.Log("SDF baked on CPU completed in "+stopwatch.Elapsed.ToString("mm\\:ss\\.ff"));
#if UNITY_EDITOR
            ...
            onBakeComplete?.Invoke( this, distancesOut, aabbMagnitude, passthrough);
        }
